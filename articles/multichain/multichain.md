...
* Liquidity inefficiencies became visible after critics claimed that user funds were transferred from an escrow wallet, to provide liquidity elsewhere. This leads to the conclusion that the SMPC nodes are tightly coordinated, and that they can control user funds. Moreover, it revealed that not all tokens in circulation (incl. IOU tokens) are also backed by funds in escrow.
...
* Related to Curve’s cross-chain gauge implementations, no severe risks were discovered.
...
Moreover, Multichain offers various levels of support to other protocols. For the adoption and implementation of new bridges, routers, or other kinds of integrations.
...
The left side lists all minted wrapped-DAI on all available chains. Roughly $106M in total. Correspondingly, the right side shows $106M DAI in escrow. However, this list is deceptive. For instance, the second line from the top right did not contain $28.5M “real” DAI. On the contrary, it contained [$28.5M Fantom-based](https://ftmscan.com/address/0xd652776de7ad802be5ec7bebfafda37600222b48) DAI, which was minted by Multichain. This was not the only case. Line four does not contain $2.3M DAI either, it contained [$2.3M anyDAI](https://optimistic.etherscan.io/address/0x4f3aff3a747fcade12598081e80c6605a8be192f) on Optimism. In other words, there is a loop effect at play, whereby minted DAI backes itself. Moreover, the article neglects to consider all circulating anyDAI. Thus, the outstanding position is actually much higher than $106M.

Our [research](https://docs.google.com/spreadsheets/d/1EEQkwUe9HSjE1rKXkFJNAcWChKbGV5Aqq-yYh2uZ5tA/edit#gid=0) shows that this is still the case. Currently, there is over $31.6M in outstanding anyDAI and over $85.6M wrapped-DAI throughout various chains. In total, we found ~$117M in circulating “artificial” DAI (wDAI + anyDAI). This corresponds with the TVL indicated on the Multichain [website](https://scan.multichain.org/#/charts/charts-home). On the other hand, we could only confirm ~$65M “real” DAI in escrow, a ~$52M difference. Note that we neglected a lot of dust wallets. So the actual amount in escrow might be higher. However, we can conclude that the outstanding IOU positions are still significantly higher than the actual DAI in escrow.

One could argue that this is not necessarily a problem, as long as there is no bank run. And even without a bank run, it can happen that users are stuck with just anyDAI. As mentioned above, the system is designed this way. In case of transfers higher than the available liquidity, users will receive anyDAI. Giving them a claim to eventually receive real DAI, once liquidity becomes available again. However, users should be aware that the ratio between escrowed DAI and “artificial” DAI is not balanced. Thus increasing the risk of getting stuck as the “last user”. It’s also worth mentioning that this report only took a closer look at DAI. Multichain has thousands of other token bridges. Users are encouraged to take a closer look at the liquidity situation when using these bridges.
