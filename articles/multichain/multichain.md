...
One could argue that the situation described above is not necessarily a problem, as long as there is no bank run. Even without a bank run, it can happen that users are stuck with just anyDAI. As mentioned above, the system is designed this way. In case of transfers higher than the available liquidity, users will receive anyDAI. Giving them a claim to eventually receive real DAI, once liquidity becomes available again. Of course, one could immediately bridge back to receive the actual DAI. However, users should be aware that the ratio between escrowed DAI and “artificial” DAI is not always balanced. Thus increasing the risk of getting stuck as the “last user”. It’s also worth mentioning that this report only took a closer look at DAI. Multichain has thousands of other token bridges. Users are encouraged to take a closer look at the liquidity situation when using these bridges.
