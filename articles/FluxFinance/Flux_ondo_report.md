...
[Ondo Finance](https://ondo.finance/) is a blockchain company that tokenizes various financial products, including tokenized securities (OUSG). Its flagship product, [OUSG](https://ondo.finance/ousg), is a tokenized security backed by the [SHV ETF](https://www.ishares.com/us/products/239466/ishares-short-treasury-bond-etf), offering users exposure to short-duration US Treasuries. OUSG has a notable attribute of being a permissioned token.

[Flux Finance](https://fluxfinance.com/) is a DeFi lending protocol developed by the technology arm of Ondo. It is designed to complement Ondo's products with additional utility. Flux Finance facilitates stablecoin loans against Ondo's tokenized US Treasuries collateral. The protocol is governed by the Ondo DAO, with discussions taking place on the governance forum and votes held on Tally.

Llama Risk conducted an extensive risk analysis on Ondo Finance and Flux Finance, revealing several risk vectors:

1. Smart Contract Risk: Ondo's smart contracts underwent an audit by Code4rena. It identified one high-severity vulnerability, five medium-severity vulnerabilities, and numerous low-severity and non-critical issues. The Ondo team worked with C4A to resolve these vulnerabilities.
2. Governance Risk: Ondo employs a two-stage governance process to ensure community involvement and reduce potential risks. Voting power distribution appears highly centralized, with two governors holding a significant share of voting power. A treasury multi-sig controls 88% of the ONDO token total supply, raising concerns about centralized decision-making.
3. Custody Risk: Ondo Finance relies on centralized exchanges like Coinbase and third-party service providers like NAV Consulting. Many of its products are held in third-party ETFs such as the Blackrock's iShares Short Treasury Bond ETF. This reliance could expose the platform to custody and regulatory risks associated with centralized institutions.
...
5. Oracle Risk: Ondo Finance uses NAV Consulting for daily price feeds and a team controlled price oracle for managing markets. The daily price change is limited at the smart contract level, reducing the risk of oracle manipulation. An integration with Chainlink is currently being tested on mainnet with an integration expected in the near future.

Although there are certainly improvements to be made, Ondo and Flux appear to be building with sustainability in mind and a careful consideration of regulatory requirements. We believe their model is a promising one to increase RWA adoption in DeFi that does not depend on speculative or risky incentive strategies. 
...
This centralization of voting power raises concerns about these entities' influence over the decision-making process within Ondo DAO's governance. Entities such as [GlassMarkets](https://www.tally.xyz/profile/glassmarkets.eth?governanceId=eip155:1:0x336505EC1BcC1A020EeDe459f57581725D23465A), for example, owns only 57 Ondo but has 894 addresses delegating vote power to it, making it the largest voter in the DAO.
